DECL|NBLE_BTWAKE_PIN|macro|NBLE_BTWAKE_PIN
DECL|NBLE_RESET_PIN|macro|NBLE_RESET_PIN
DECL|NBLE_SWDIO_PIN|macro|NBLE_SWDIO_PIN
DECL|bdaddr|variable|bdaddr
DECL|bt_addr_le_str|function|static const char *bt_addr_le_str(const bt_addr_le_t *addr)
DECL|bt_enable|function|int bt_enable(bt_ready_cb_t cb)
DECL|bt_le_adv_start|function|int bt_le_adv_start(const struct bt_le_adv_param *param, const struct bt_data *ad, size_t ad_len, const struct bt_data *sd, size_t sd_len)
DECL|bt_le_adv_stop|function|int bt_le_adv_stop(void)
DECL|bt_le_scan_start|function|int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
DECL|bt_le_scan_stop|function|int bt_le_scan_stop(void)
DECL|bt_ready_cb|variable|bt_ready_cb
DECL|nble_log|function|void nble_log(const struct nble_log_s *param, char *format, uint8_t len)
DECL|on_ble_set_enable_config_rsp|function|void on_ble_set_enable_config_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_cancel_connect_rsp|function|void on_nble_gap_cancel_connect_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_clr_white_list_rsp|function|void on_nble_gap_clr_white_list_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_conn_update_rsp|function|void on_nble_gap_conn_update_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_connect_rsp|function|void on_nble_gap_connect_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_dbg_rsp|function|void on_nble_gap_dbg_rsp(const struct debug_response *rsp)
DECL|on_nble_gap_disconnect_rsp|function|void on_nble_gap_disconnect_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_dtm_init_rsp|function|void on_nble_gap_dtm_init_rsp(void *user_data)
DECL|on_nble_gap_generic_cmd_rsp|function|void on_nble_gap_generic_cmd_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_read_bda_rsp|function|void on_nble_gap_read_bda_rsp(const struct nble_service_read_bda_response *rsp)
DECL|on_nble_gap_rssi_evt|function|void on_nble_gap_rssi_evt(const struct nble_gap_rssi_evt *ev)
DECL|on_nble_gap_service_read_rsp|function|void on_nble_gap_service_read_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_service_write_rsp|function|void on_nble_gap_service_write_rsp(const struct nble_service_write_response *rsp)
DECL|on_nble_gap_set_option_rsp|function|void on_nble_gap_set_option_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_set_rssi_report_rsp|function|void on_nble_gap_set_rssi_report_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_sm_clear_bonds_rsp|function|void on_nble_gap_sm_clear_bonds_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_sm_config_rsp|function|void on_nble_gap_sm_config_rsp(struct nble_gap_sm_config_rsp *rsp)
DECL|on_nble_gap_sm_pairing_rsp|function|void on_nble_gap_sm_pairing_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_sm_passkey_display_evt|function|void on_nble_gap_sm_passkey_display_evt(const struct nble_gap_sm_passkey_disp_evt *ev)
DECL|on_nble_gap_sm_passkey_reply_rsp|function|void on_nble_gap_sm_passkey_reply_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_sm_passkey_req_evt|function|void on_nble_gap_sm_passkey_req_evt(const struct nble_gap_sm_passkey_req_evt *ev)
DECL|on_nble_gap_sm_status_evt|function|void on_nble_gap_sm_status_evt(const struct nble_gap_sm_status_evt *ev)
DECL|on_nble_gap_start_advertise_rsp|function|void on_nble_gap_start_advertise_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_start_scan_rsp|function|void on_nble_gap_start_scan_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_stop_advertise_rsp|function|void on_nble_gap_stop_advertise_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_stop_scan_rsp|function|void on_nble_gap_stop_scan_rsp(const struct nble_response *rsp)
DECL|on_nble_gap_to_evt|function|void on_nble_gap_to_evt(const struct nble_gap_timout_evt *ev)
DECL|on_nble_gap_wr_white_list_rsp|function|void on_nble_gap_wr_white_list_rsp(const struct nble_response *rsp)
DECL|on_nble_get_version_rsp|function|void on_nble_get_version_rsp(const struct nble_version_response *rsp)
DECL|on_nble_up|function|void on_nble_up(void)
DECL|send_dm_config|function|static void send_dm_config(void)
DECL|valid_adv_param|function|static bool valid_adv_param(const struct bt_le_adv_param *param)

...
DECL|dw_read|function|static inline uint32_t dw_read(uint32_t base_addr, uint32_t offest)
DECL|dw_set_bit|function|static void dw_set_bit(uint32_t base_addr, uint32_t offset, uint32_t bit, uint8_t value)
DECL|dw_write|function|static inline void dw_write(uint32_t base_addr, uint32_t offest, uint32_t val)

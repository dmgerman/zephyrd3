DECL|ADV_CHAN_MAP|macro|ADV_CHAN_MAP
DECL|ADV_INTERVAL|macro|ADV_INTERVAL
DECL|ADV_PHY_S|macro|ADV_PHY_S
DECL|ADV_SEC_SKIP|macro|ADV_SEC_SKIP
DECL|ADV_SID|macro|ADV_SID
DECL|ADV_TX_PWR|macro|ADV_TX_PWR
DECL|ADV_TYPE|macro|ADV_TYPE
DECL|FILTER_POLICY|macro|FILTER_POLICY
DECL|OWN_ADDR_TYPE|macro|OWN_ADDR_TYPE
DECL|PEER_ADDR_TYPE|macro|PEER_ADDR_TYPE
DECL|PEER_ADDR|macro|PEER_ADDR
DECL|SCAN_FILTER_POLICY|macro|SCAN_FILTER_POLICY
DECL|SCAN_INTERVAL|macro|SCAN_INTERVAL
DECL|SCAN_OWN_ADDR_TYPE|macro|SCAN_OWN_ADDR_TYPE
DECL|SCAN_REQ_NOT|macro|SCAN_REQ_NOT
DECL|SCAN_WINDOW|macro|SCAN_WINDOW
DECL|cmd_advx|function|int cmd_advx(const struct shell *shell, size_t argc, char *argv[])
DECL|cmd_ll_addr_get|function|int cmd_ll_addr_get(const struct shell *shell, size_t argc, char *argv[])
DECL|cmd_scanx|function|int cmd_scanx(const struct shell *shell, size_t argc, char *argv[])
DECL|cmd_test_end|function|int cmd_test_end(const struct shell *shell, size_t argc, char *argv[])
DECL|cmd_test_rx|function|int cmd_test_rx(const struct shell *shell, size_t argc, char *argv[])
DECL|cmd_test_tx|function|int cmd_test_tx(const struct shell *shell, size_t argc, char *argv[])

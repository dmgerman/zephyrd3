...
DECL|rx_buf|member|uint8_t rx_buf[WEBUSB_BUFFER_SIZE];/* Internal Rx buffer */
...
DECL|webusb_serial_baudrate_set|function|static void webusb_serial_baudrate_set(struct device *dev, uint32_t baudrate)
DECL|webusb_serial_bulk_in|function|static void webusb_serial_bulk_in(uint8_t ep,enum usb_dc_ep_cb_status_code ep_status)
DECL|webusb_serial_bulk_out|function|static void webusb_serial_bulk_out(uint8_t ep,enum usb_dc_ep_cb_status_code ep_status)
DECL|webusb_serial_class_handle_req|function|int webusb_serial_class_handle_req(struct usb_setup_packet *pSetup,int32_t *len, uint8_t **data)
DECL|webusb_serial_config|variable|webusb_serial_config
DECL|webusb_serial_custom_handle_req|function|int webusb_serial_custom_handle_req(struct usb_setup_packet *pSetup,int32_t *len, uint8_t **data)
...
DECL|webusb_serial_fifo_fill|function|static int webusb_serial_fifo_fill(struct device *dev,const uint8_t *tx_data, int len)
DECL|webusb_serial_fifo_read|function|static int webusb_serial_fifo_read(struct device *dev, uint8_t *rx_data,const int size)
DECL|webusb_serial_init|function|static int webusb_serial_init(struct device *dev)
DECL|webusb_serial_int_in|function|static void webusb_serial_int_in(uint8_t ep,enum usb_dc_ep_cb_status_code ep_status)
...
DECL|webusb_serial_line_ctrl_get|function|static int webusb_serial_line_ctrl_get(struct device *dev,uint32_t ctrl, uint32_t *val)
DECL|webusb_serial_line_ctrl_set|function|static int webusb_serial_line_ctrl_set(struct device *dev,uint32_t ctrl, uint32_t val)
...
DECL|webusb_serial_send_notification|function|static int webusb_serial_send_notification(struct device *dev,uint16_t serial_state)
DECL|webusb_serial_usb_description|variable|webusb_serial_usb_description
DECL|webusb_serial_vendor_handle_req|function|int webusb_serial_vendor_handle_req(struct usb_setup_packet *pSetup,int32_t *len, uint8_t **data)

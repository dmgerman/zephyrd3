...
DECL|http_rx_tx|function|void http_rx_tx(struct net_context *net_ctx, struct net_buf *rx, int status,void *user_data)
DECL|http_tx|function|static void http_tx(struct http_server_ctx *http_ctx)
DECL|http_url_add|function|int http_url_add(const char *url, uint8_t flags, int (*write_cb)(struct http_server_ctx *http_ctx))
DECL|http_url_cmp|function|static int http_url_cmp(const char *url, uint16_t url_len,const char *root_url, uint16_t root_url_len)

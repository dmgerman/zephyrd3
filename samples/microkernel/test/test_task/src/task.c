DECL|CMD_PRIORITY|macro|CMD_PRIORITY
DECL|CMD_TASKID|macro|CMD_TASKID
DECL|HT_PRIO|macro|HT_PRIO
DECL|HelperTask|function|void HelperTask (void)
DECL|ISR_INFO|typedef|} ISR_INFO;
DECL|NUM_SW_IRQS|macro|NUM_SW_IRQS
DECL|RT_PRIO|macro|RT_PRIO
DECL|RegressionTask|function|void RegressionTask (void)
DECL|SLEEP_TIME|macro|SLEEP_TIME
DECL|_trigger_isrTaskCommand|variable|_trigger_isrTaskCommand
DECL|cmd|member|int cmd;
DECL|data|member|int data;
DECL|helperData|variable|helperData
DECL|helperTaskSetPrioTest|function|void helperTaskSetPrioTest (void)
DECL|helperTaskSleepTest|function|void helperTaskSleepTest (void)
DECL|helperTaskSuspendTest|function|void helperTaskSuspendTest (void)
DECL|helperTaskYieldTest|function|void helperTaskYieldTest (void)
DECL|isrAPIsTest|function|int isrAPIsTest( int taskId, int taskPrio )
DECL|isrInfo|variable|isrInfo
DECL|isr_task_command_handler|function|void isr_task_command_handler( void * data )
DECL|mainTaskNotReady|variable|mainTaskNotReady
DECL|microObjectsInit|function|void microObjectsInit (void)
DECL|taskMacrosTest|function|int taskMacrosTest( int taskId, int taskPrio )
DECL|taskSetPrioTest|function|int taskSetPrioTest (void)
DECL|taskSleepTest|function|int taskSleepTest (void)
DECL|taskSuspendTest|function|int taskSuspendTest (void)
DECL|taskYieldTest|function|int taskYieldTest (void)
DECL|tcRC|variable|tcRC

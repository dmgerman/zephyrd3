DECL|COMP_EXTI_EVENT|macro|COMP_EXTI_EVENT
DECL|COMP_EXTI_FALLING|macro|COMP_EXTI_FALLING
DECL|COMP_EXTI_IT|macro|COMP_EXTI_IT
DECL|COMP_EXTI_LINE_COMP1|macro|COMP_EXTI_LINE_COMP1
DECL|COMP_EXTI_LINE_COMP2|macro|COMP_EXTI_LINE_COMP2
DECL|COMP_EXTI_RISING|macro|COMP_EXTI_RISING
DECL|COMP_GET_EXTI_LINE|macro|COMP_GET_EXTI_LINE
DECL|COMP_HandleTypeDef|typedef|} COMP_HandleTypeDef;
DECL|COMP_INPUT_MINUS_1_2VREFINT|macro|COMP_INPUT_MINUS_1_2VREFINT
DECL|COMP_INPUT_MINUS_1_4VREFINT|macro|COMP_INPUT_MINUS_1_4VREFINT
DECL|COMP_INPUT_MINUS_3_4VREFINT|macro|COMP_INPUT_MINUS_3_4VREFINT
DECL|COMP_INPUT_MINUS_DAC1_CH1|macro|COMP_INPUT_MINUS_DAC1_CH1
DECL|COMP_INPUT_MINUS_DAC1_CH2|macro|COMP_INPUT_MINUS_DAC1_CH2
DECL|COMP_INPUT_MINUS_IO1|macro|COMP_INPUT_MINUS_IO1
DECL|COMP_INPUT_MINUS_IO2|macro|COMP_INPUT_MINUS_IO2
DECL|COMP_INPUT_MINUS_VREFINT|macro|COMP_INPUT_MINUS_VREFINT
DECL|COMP_INPUT_PLUS_IO1|macro|COMP_INPUT_PLUS_IO1
DECL|COMP_INPUT_PLUS_IO2|macro|COMP_INPUT_PLUS_IO2
DECL|COMP_INPUT_PLUS_IO3|macro|COMP_INPUT_PLUS_IO3
DECL|COMP_INPUT_PLUS_IO4|macro|COMP_INPUT_PLUS_IO4
DECL|COMP_INPUT_PLUS_IO5|macro|COMP_INPUT_PLUS_IO5
DECL|COMP_INPUT_PLUS_IO6|macro|COMP_INPUT_PLUS_IO6
DECL|COMP_InitTypeDef|typedef|}COMP_InitTypeDef;
DECL|COMP_LPTIMCONNECTION_DISABLED|macro|COMP_LPTIMCONNECTION_DISABLED
DECL|COMP_LPTIMCONNECTION_IN1_ENABLED|macro|COMP_LPTIMCONNECTION_IN1_ENABLED
DECL|COMP_LPTIMCONNECTION_IN2_ENABLED|macro|COMP_LPTIMCONNECTION_IN2_ENABLED
DECL|COMP_OUTPUTPOL_INVERTED|macro|COMP_OUTPUTPOL_INVERTED
DECL|COMP_OUTPUTPOL_NONINVERTED|macro|COMP_OUTPUTPOL_NONINVERTED
DECL|COMP_OUTPUT_LEVEL_HIGH|macro|COMP_OUTPUT_LEVEL_HIGH
DECL|COMP_OUTPUT_LEVEL_LOW|macro|COMP_OUTPUT_LEVEL_LOW
DECL|COMP_POWERMODE_MEDIUMSPEED|macro|COMP_POWERMODE_MEDIUMSPEED
DECL|COMP_POWERMODE_ULTRALOWPOWER|macro|COMP_POWERMODE_ULTRALOWPOWER
DECL|COMP_STATE_BITFIELD_LOCK|macro|COMP_STATE_BITFIELD_LOCK
DECL|COMP_TRIGGERMODE_EVENT_FALLING|macro|COMP_TRIGGERMODE_EVENT_FALLING
DECL|COMP_TRIGGERMODE_EVENT_RISING_FALLING|macro|COMP_TRIGGERMODE_EVENT_RISING_FALLING
DECL|COMP_TRIGGERMODE_EVENT_RISING|macro|COMP_TRIGGERMODE_EVENT_RISING
DECL|COMP_TRIGGERMODE_IT_FALLING|macro|COMP_TRIGGERMODE_IT_FALLING
DECL|COMP_TRIGGERMODE_IT_RISING_FALLING|macro|COMP_TRIGGERMODE_IT_RISING_FALLING
DECL|COMP_TRIGGERMODE_IT_RISING|macro|COMP_TRIGGERMODE_IT_RISING
DECL|COMP_TRIGGERMODE_NONE|macro|COMP_TRIGGERMODE_NONE
DECL|COMP_WINDOWMODE_COMP1_INPUT_PLUS_COMMON|macro|COMP_WINDOWMODE_COMP1_INPUT_PLUS_COMMON
DECL|COMP_WINDOWMODE_DISABLE|macro|COMP_WINDOWMODE_DISABLE
DECL|HAL_COMP_STATE_BUSY_LOCKED|enumerator|HAL_COMP_STATE_BUSY_LOCKED = (HAL_COMP_STATE_BUSY | COMP_STATE_BITFIELD_LOCK) /*!< COMP is running and configuration is locked */
DECL|HAL_COMP_STATE_BUSY|enumerator|HAL_COMP_STATE_BUSY = 0x02U, /*!< COMP is running */
DECL|HAL_COMP_STATE_READY_LOCKED|enumerator|HAL_COMP_STATE_READY_LOCKED = (HAL_COMP_STATE_READY | COMP_STATE_BITFIELD_LOCK), /*!< COMP initialized but configuration is locked */
DECL|HAL_COMP_STATE_READY|enumerator|HAL_COMP_STATE_READY = 0x01U, /*!< COMP initialized and ready for use */
DECL|HAL_COMP_STATE_RESET_LOCKED|enumerator|HAL_COMP_STATE_RESET_LOCKED = (HAL_COMP_STATE_RESET | COMP_STATE_BITFIELD_LOCK), /*!< COMP not yet initialized and configuration is locked */
DECL|HAL_COMP_STATE_RESET|enumerator|HAL_COMP_STATE_RESET = 0x00U, /*!< COMP not yet initialized */
DECL|HAL_COMP_StateTypeDef|typedef|}HAL_COMP_StateTypeDef;
DECL|IS_COMP1_LPTIMCONNECTION|macro|IS_COMP1_LPTIMCONNECTION
DECL|IS_COMP2_LPTIMCONNECTION_RESTRICTED|macro|IS_COMP2_LPTIMCONNECTION_RESTRICTED
DECL|IS_COMP2_LPTIMCONNECTION|macro|IS_COMP2_LPTIMCONNECTION
DECL|IS_COMP_INPUT_MINUS|macro|IS_COMP_INPUT_MINUS
DECL|IS_COMP_INPUT_PLUS|macro|IS_COMP_INPUT_PLUS
DECL|IS_COMP_INPUT_PLUS|macro|IS_COMP_INPUT_PLUS
DECL|IS_COMP_OUTPUTPOL|macro|IS_COMP_OUTPUTPOL
DECL|IS_COMP_OUTPUT_LEVEL|macro|IS_COMP_OUTPUT_LEVEL
DECL|IS_COMP_POWERMODE|macro|IS_COMP_POWERMODE
DECL|IS_COMP_TRIGGERMODE|macro|IS_COMP_TRIGGERMODE
DECL|IS_COMP_WINDOWMODE_INSTANCE|macro|IS_COMP_WINDOWMODE_INSTANCE
DECL|IS_COMP_WINDOWMODE|macro|IS_COMP_WINDOWMODE
DECL|Init|member|COMP_InitTypeDef Init; /*!< COMP required parameters */
DECL|Instance|member|COMP_TypeDef *Instance; /*!< Register base address */
DECL|InvertingInput|member|uint32_t InvertingInput; /*!< Set comparator input minus (inverting input).
DECL|LPTIMConnection|member|uint32_t LPTIMConnection; /*!< Set comparator output connection to LPTIM peripheral.
DECL|Lock|member|HAL_LockTypeDef Lock; /*!< Locking object */
DECL|Mode|member|uint32_t Mode; /*!< Set comparator operating mode to adjust power and speed.
DECL|NonInvertingInput|member|uint32_t NonInvertingInput; /*!< Set comparator input plus (non-inverting input).
DECL|OutputPol|member|uint32_t OutputPol; /*!< Set comparator output polarity.
DECL|State|member|__IO HAL_COMP_StateTypeDef State; /*!< COMP communication state */
DECL|TriggerMode|member|uint32_t TriggerMode; /*!< Set the comparator output triggering External Interrupt Line (EXTI).
DECL|WindowMode|member|uint32_t WindowMode; /*!< Set window mode of a pair of comparators instances
DECL|__HAL_COMP_COMP1_EXTI_CLEAR_FLAG|macro|__HAL_COMP_COMP1_EXTI_CLEAR_FLAG
DECL|__HAL_COMP_COMP1_EXTI_DISABLE_EVENT|macro|__HAL_COMP_COMP1_EXTI_DISABLE_EVENT
DECL|__HAL_COMP_COMP1_EXTI_DISABLE_FALLING_EDGE|macro|__HAL_COMP_COMP1_EXTI_DISABLE_FALLING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_DISABLE_IT|macro|__HAL_COMP_COMP1_EXTI_DISABLE_IT
DECL|__HAL_COMP_COMP1_EXTI_DISABLE_RISING_EDGE|macro|__HAL_COMP_COMP1_EXTI_DISABLE_RISING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_DISABLE_RISING_FALLING_EDGE|macro|__HAL_COMP_COMP1_EXTI_DISABLE_RISING_FALLING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_ENABLE_EVENT|macro|__HAL_COMP_COMP1_EXTI_ENABLE_EVENT
DECL|__HAL_COMP_COMP1_EXTI_ENABLE_FALLING_EDGE|macro|__HAL_COMP_COMP1_EXTI_ENABLE_FALLING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_ENABLE_IT|macro|__HAL_COMP_COMP1_EXTI_ENABLE_IT
DECL|__HAL_COMP_COMP1_EXTI_ENABLE_RISING_EDGE|macro|__HAL_COMP_COMP1_EXTI_ENABLE_RISING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_ENABLE_RISING_FALLING_EDGE|macro|__HAL_COMP_COMP1_EXTI_ENABLE_RISING_FALLING_EDGE
DECL|__HAL_COMP_COMP1_EXTI_GENERATE_SWIT|macro|__HAL_COMP_COMP1_EXTI_GENERATE_SWIT
DECL|__HAL_COMP_COMP1_EXTI_GET_FLAG|macro|__HAL_COMP_COMP1_EXTI_GET_FLAG
DECL|__HAL_COMP_COMP2_EXTI_CLEAR_FLAG|macro|__HAL_COMP_COMP2_EXTI_CLEAR_FLAG
DECL|__HAL_COMP_COMP2_EXTI_DISABLE_EVENT|macro|__HAL_COMP_COMP2_EXTI_DISABLE_EVENT
DECL|__HAL_COMP_COMP2_EXTI_DISABLE_FALLING_EDGE|macro|__HAL_COMP_COMP2_EXTI_DISABLE_FALLING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_DISABLE_IT|macro|__HAL_COMP_COMP2_EXTI_DISABLE_IT
DECL|__HAL_COMP_COMP2_EXTI_DISABLE_RISING_EDGE|macro|__HAL_COMP_COMP2_EXTI_DISABLE_RISING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_DISABLE_RISING_FALLING_EDGE|macro|__HAL_COMP_COMP2_EXTI_DISABLE_RISING_FALLING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_ENABLE_EVENT|macro|__HAL_COMP_COMP2_EXTI_ENABLE_EVENT
DECL|__HAL_COMP_COMP2_EXTI_ENABLE_FALLING_EDGE|macro|__HAL_COMP_COMP2_EXTI_ENABLE_FALLING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_ENABLE_IT|macro|__HAL_COMP_COMP2_EXTI_ENABLE_IT
DECL|__HAL_COMP_COMP2_EXTI_ENABLE_RISING_EDGE|macro|__HAL_COMP_COMP2_EXTI_ENABLE_RISING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_ENABLE_RISING_FALLING_EDGE|macro|__HAL_COMP_COMP2_EXTI_ENABLE_RISING_FALLING_EDGE
DECL|__HAL_COMP_COMP2_EXTI_GENERATE_SWIT|macro|__HAL_COMP_COMP2_EXTI_GENERATE_SWIT
DECL|__HAL_COMP_COMP2_EXTI_GET_FLAG|macro|__HAL_COMP_COMP2_EXTI_GET_FLAG
DECL|__HAL_COMP_DISABLE|macro|__HAL_COMP_DISABLE
DECL|__HAL_COMP_ENABLE|macro|__HAL_COMP_ENABLE
DECL|__HAL_COMP_IS_LOCKED|macro|__HAL_COMP_IS_LOCKED
DECL|__HAL_COMP_LOCK|macro|__HAL_COMP_LOCK
DECL|__HAL_COMP_RESET_HANDLE_STATE|macro|__HAL_COMP_RESET_HANDLE_STATE
DECL|__STM32L0xx_HAL_COMP_H|macro|__STM32L0xx_HAL_COMP_H

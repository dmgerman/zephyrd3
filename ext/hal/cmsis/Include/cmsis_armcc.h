DECL|__ALIGNED|macro|__ALIGNED
DECL|__ARM_ARCH_6M__|macro|__ARM_ARCH_6M__
DECL|__ARM_ARCH_7EM__|macro|__ARM_ARCH_7EM__
DECL|__ARM_ARCH_7M__|macro|__ARM_ARCH_7M__
DECL|__ASM|macro|__ASM
DECL|__BKPT|macro|__BKPT
DECL|__CLREX|macro|__CLREX
DECL|__CLZ|macro|__CLZ
DECL|__CMSIS_ARMCC_H|macro|__CMSIS_ARMCC_H
DECL|__DMB|macro|__DMB
DECL|__DSB|macro|__DSB
DECL|__INLINE|macro|__INLINE
DECL|__ISB|macro|__ISB
DECL|__LDRBT|macro|__LDRBT
DECL|__LDREXB|macro|__LDREXB
DECL|__LDREXB|macro|__LDREXB
DECL|__LDREXH|macro|__LDREXH
DECL|__LDREXH|macro|__LDREXH
DECL|__LDREXW|macro|__LDREXW
DECL|__LDREXW|macro|__LDREXW
DECL|__LDRHT|macro|__LDRHT
DECL|__LDRT|macro|__LDRT
DECL|__NOP|macro|__NOP
DECL|__NO_RETURN|macro|__NO_RETURN
DECL|__PACKED_STRUCT|macro|__PACKED_STRUCT
DECL|__PACKED_UNION|macro|__PACKED_UNION
DECL|__PACKED|macro|__PACKED
DECL|__PKHBT|macro|__PKHBT
DECL|__PKHTB|macro|__PKHTB
DECL|__QADD16|macro|__QADD16
DECL|__QADD8|macro|__QADD8
DECL|__QADD|macro|__QADD
DECL|__QASX|macro|__QASX
DECL|__QSAX|macro|__QSAX
DECL|__QSUB16|macro|__QSUB16
DECL|__QSUB8|macro|__QSUB8
DECL|__QSUB|macro|__QSUB
DECL|__RBIT|function|__attribute__((always_inline)) __STATIC_INLINE uint32_t __RBIT(uint32_t value)
DECL|__RBIT|macro|__RBIT
DECL|__RESTRICT|macro|__RESTRICT
DECL|__REV16|function|__attribute__((section(".rev16_text"))) __STATIC_INLINE __ASM uint32_t __REV16(uint32_t value)
DECL|__REVSH|function|__attribute__((section(".revsh_text"))) __STATIC_INLINE __ASM int16_t __REVSH(int16_t value)
DECL|__REV|macro|__REV
DECL|__ROR|macro|__ROR
DECL|__RRX|function|__attribute__((section(".rrx_text"))) __STATIC_INLINE __ASM uint32_t __RRX(uint32_t value)
DECL|__SADD16|macro|__SADD16
DECL|__SADD8|macro|__SADD8
DECL|__SASX|macro|__SASX
DECL|__SEL|macro|__SEL
DECL|__SEV|macro|__SEV
DECL|__SHADD16|macro|__SHADD16
DECL|__SHADD8|macro|__SHADD8
DECL|__SHASX|macro|__SHASX
DECL|__SHSAX|macro|__SHSAX
DECL|__SHSUB16|macro|__SHSUB16
DECL|__SHSUB8|macro|__SHSUB8
DECL|__SMLADX|macro|__SMLADX
DECL|__SMLAD|macro|__SMLAD
DECL|__SMLALDX|macro|__SMLALDX
DECL|__SMLALD|macro|__SMLALD
DECL|__SMLSDX|macro|__SMLSDX
DECL|__SMLSD|macro|__SMLSD
DECL|__SMLSLDX|macro|__SMLSLDX
DECL|__SMLSLD|macro|__SMLSLD
DECL|__SMMLA|macro|__SMMLA
DECL|__SMUADX|macro|__SMUADX
DECL|__SMUAD|macro|__SMUAD
DECL|__SMUSDX|macro|__SMUSDX
DECL|__SMUSD|macro|__SMUSD
DECL|__SSAT16|macro|__SSAT16
DECL|__SSAT|function|__attribute__((always_inline)) __STATIC_INLINE int32_t __SSAT(int32_t val, uint32_t sat)
DECL|__SSAT|macro|__SSAT
DECL|__SSAX|macro|__SSAX
DECL|__SSUB16|macro|__SSUB16
DECL|__SSUB8|macro|__SSUB8
DECL|__STATIC_FORCEINLINE|macro|__STATIC_FORCEINLINE
DECL|__STATIC_INLINE|macro|__STATIC_INLINE
DECL|__STRBT|macro|__STRBT
DECL|__STREXB|macro|__STREXB
DECL|__STREXB|macro|__STREXB
DECL|__STREXH|macro|__STREXH
DECL|__STREXH|macro|__STREXH
DECL|__STREXW|macro|__STREXW
DECL|__STREXW|macro|__STREXW
DECL|__STRHT|macro|__STRHT
DECL|__STRT|macro|__STRT
DECL|__SXTAB16|macro|__SXTAB16
DECL|__SXTB16|macro|__SXTB16
DECL|__UADD16|macro|__UADD16
DECL|__UADD8|macro|__UADD8
DECL|__UASX|macro|__UASX
DECL|__UHADD16|macro|__UHADD16
DECL|__UHADD8|macro|__UHADD8
DECL|__UHASX|macro|__UHASX
DECL|__UHSAX|macro|__UHSAX
DECL|__UHSUB16|macro|__UHSUB16
DECL|__UHSUB8|macro|__UHSUB8
DECL|__UNALIGNED_UINT16_READ|macro|__UNALIGNED_UINT16_READ
DECL|__UNALIGNED_UINT16_WRITE|macro|__UNALIGNED_UINT16_WRITE
DECL|__UNALIGNED_UINT32_READ|macro|__UNALIGNED_UINT32_READ
DECL|__UNALIGNED_UINT32_WRITE|macro|__UNALIGNED_UINT32_WRITE
DECL|__UNALIGNED_UINT32|macro|__UNALIGNED_UINT32
DECL|__UQADD16|macro|__UQADD16
DECL|__UQADD8|macro|__UQADD8
DECL|__UQASX|macro|__UQASX
DECL|__UQSAX|macro|__UQSAX
DECL|__UQSUB16|macro|__UQSUB16
DECL|__UQSUB8|macro|__UQSUB8
DECL|__USAD8|macro|__USAD8
DECL|__USADA8|macro|__USADA8
DECL|__USAT16|macro|__USAT16
DECL|__USAT|function|__attribute__((always_inline)) __STATIC_INLINE uint32_t __USAT(int32_t val, uint32_t sat)
DECL|__USAT|macro|__USAT
DECL|__USAX|macro|__USAX
DECL|__USED|macro|__USED
DECL|__USUB16|macro|__USUB16
DECL|__USUB8|macro|__USUB8
DECL|__UXTAB16|macro|__UXTAB16
DECL|__UXTB16|macro|__UXTB16
DECL|__WEAK|macro|__WEAK
DECL|__WFE|macro|__WFE
DECL|__WFI|macro|__WFI
DECL|__disable_fault_irq|macro|__disable_fault_irq
DECL|__enable_fault_irq|macro|__enable_fault_irq
DECL|__get_APSR|function|__STATIC_INLINE uint32_t __get_APSR(void)
DECL|__get_BASEPRI|function|__STATIC_INLINE uint32_t __get_BASEPRI(void)
DECL|__get_CONTROL|function|__STATIC_INLINE uint32_t __get_CONTROL(void)
DECL|__get_FAULTMASK|function|__STATIC_INLINE uint32_t __get_FAULTMASK(void)
DECL|__get_FPSCR|function|__STATIC_INLINE uint32_t __get_FPSCR(void)
DECL|__get_IPSR|function|__STATIC_INLINE uint32_t __get_IPSR(void)
DECL|__get_MSP|function|__STATIC_INLINE uint32_t __get_MSP(void)
DECL|__get_PRIMASK|function|__STATIC_INLINE uint32_t __get_PRIMASK(void)
DECL|__get_PSP|function|__STATIC_INLINE uint32_t __get_PSP(void)
DECL|__get_xPSR|function|__STATIC_INLINE uint32_t __get_xPSR(void)
DECL|__set_BASEPRI_MAX|function|__STATIC_INLINE void __set_BASEPRI_MAX(uint32_t basePri)
DECL|__set_BASEPRI|function|__STATIC_INLINE void __set_BASEPRI(uint32_t basePri)
DECL|__set_CONTROL|function|__STATIC_INLINE void __set_CONTROL(uint32_t control)
DECL|__set_FAULTMASK|function|__STATIC_INLINE void __set_FAULTMASK(uint32_t faultMask)
DECL|__set_FPSCR|function|__STATIC_INLINE void __set_FPSCR(uint32_t fpscr)
DECL|__set_MSP|function|__STATIC_INLINE void __set_MSP(uint32_t topOfMainStack)
DECL|__set_PRIMASK|function|__STATIC_INLINE void __set_PRIMASK(uint32_t priMask)
DECL|__set_PSP|function|__STATIC_INLINE void __set_PSP(uint32_t topOfProcStack)

DECL|BOOT_BANNER|macro|BOOT_BANNER
DECL|BUILD_TIMESTAMP|macro|BUILD_TIMESTAMP
DECL|PRINT_BOOT_BANNER|macro|PRINT_BOOT_BANNER
DECL|PRINT_BOOT_BANNER|macro|PRINT_BOOT_BANNER
DECL|_FiberStart|function|void _FiberStart(char *pStack, unsigned stackSize, /* stack size in bytes */ nano_fiber_entry_t pEntry, int parameter1, int parameter2,
DECL|_InsertCCS|function|void _InsertCCS(tCCS **queue, tCCS *ccs)
DECL|_NanoFiberSwap|function|FUNC_NORETURN void _NanoFiberSwap(void)
DECL|_NanoStart|function|void _NanoStart(void)
DECL|_context_essential_check|function|int _context_essential_check(tCCS *pCtx /* pointer to context */ )
DECL|build_timestamp|variable|build_timestamp
DECL|context_self_get|function|nano_context_id_t context_self_get(void)
DECL|context_type_get|function|nano_context_type_t context_type_get(void)
DECL|fiber_abort|function|FUNC_NORETURN void fiber_abort(void)
DECL|fiber_yield|function|void fiber_yield(void)

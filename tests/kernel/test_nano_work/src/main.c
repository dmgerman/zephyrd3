DECL|FIBER_STACK_SIZE|macro|FIBER_STACK_SIZE
DECL|NUM_TEST_ITEMS|macro|NUM_TEST_ITEMS
DECL|SUBMIT_WAIT|macro|SUBMIT_WAIT
DECL|WORK_ITEM_WAIT|macro|WORK_ITEM_WAIT
DECL|check_results|function|static int check_results(int num_tests)
DECL|delayed_resubmit_work_handler|function|static void delayed_resubmit_work_handler(struct nano_work *work)
DECL|delayed_work_handler|function|static void delayed_work_handler(struct nano_work *work)
DECL|fiber_delayed_work_cancel_main|function|static void fiber_delayed_work_cancel_main(int arg1, int arg2)
DECL|fiber_delayed_work_main|function|static void fiber_delayed_work_main(int arg1, int arg2)
DECL|fiber_delayed_work_resubmit|function|static void fiber_delayed_work_resubmit(int arg1, int arg2)
DECL|fiber_stack|variable|fiber_stack
DECL|fiber_work_main|function|static void fiber_work_main(int arg1, int arg2)
DECL|key|member|int key;
DECL|main|function|void main(void)
DECL|num_results|variable|num_results
DECL|reset_results|function|static void reset_results(void)
DECL|resubmit_work_handler|function|static void resubmit_work_handler(struct nano_work *work)
DECL|results|variable|results
DECL|test_delayed_cancel|function|static int test_delayed_cancel(void)
DECL|test_delayed_init|function|static void test_delayed_init(void)
DECL|test_delayed_resubmit_fiber|function|static int test_delayed_resubmit_fiber(void)
DECL|test_delayed_resubmit|function|static int test_delayed_resubmit(void)
DECL|test_delayed_submit|function|static int test_delayed_submit(void)
DECL|test_delayed|function|static int test_delayed(void)
DECL|test_items_init|function|static void test_items_init(void)
DECL|test_items_submit|function|static void test_items_submit(void)
DECL|test_item|struct|struct test_item {
DECL|test_resubmit|function|static int test_resubmit(void)
DECL|test_sequence|function|static int test_sequence(void)
DECL|tests|variable|tests
DECL|work_handler|function|static void work_handler(struct nano_work *work)
DECL|work|member|struct nano_delayed_work work;

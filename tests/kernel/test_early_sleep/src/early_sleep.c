DECL|AlternateTask|function|void AlternateTask(void)
DECL|FIBER_TICKS_TO_SLEEP|macro|FIBER_TICKS_TO_SLEEP
DECL|RegressionTask|function|void RegressionTask(void)
DECL|STACKSIZE|macro|STACKSIZE
DECL|TASK_TICKS_TO_SLEEP|macro|TASK_TICKS_TO_SLEEP
DECL|absdiff|function|static inline int absdiff(int x, int y)
DECL|alternate_task_run|variable|alternate_task_run
DECL|fiber_actual_sleep_ticks|variable|fiber_actual_sleep_ticks
DECL|task_actual_sleep_ticks|variable|task_actual_sleep_ticks
DECL|test_early_task_sleep|function|static int test_early_task_sleep(struct device *unused)
DECL|test_fiber_sem|variable|test_fiber_sem
DECL|test_fiber_sleep|function|int test_fiber_sleep(int ticks_to_sleep)
DECL|test_fiber_stack|variable|test_fiber_stack
DECL|test_fiber_start|function|static int test_fiber_start(struct device *unused)
DECL|test_fiber|function|static void test_fiber(int ticks_to_sleep, int unused)
DECL|test_task_sleep|function|int test_task_sleep(int ticks_to_sleep)

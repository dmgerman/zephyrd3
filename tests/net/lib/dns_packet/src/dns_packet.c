...
DECL|ancount|member|uint8_t ancount;
DECL|answer_type|member|uint8_t answer_type;
...
DECL|eval_query|function|static int eval_query(const char *dname, uint16_t tid, enum dns_rr_type type, uint8_t *expected, uint16_t expected_len)
...
DECL|ra|member|uint8_t ra;
DECL|rdata|member|const uint8_t *rdata;
DECL|rdlen|member|uint8_t rdlen;
DECL|res_len|member|uint16_t res_len;

DECL|IRQ_LINE|macro|IRQ_LINE
DECL|IRQ_PRIORITY|macro|IRQ_PRIORITY
DECL|MESSAGE|macro|MESSAGE
DECL|TEST_IRQ_OFFLOAD_VECTOR|macro|TEST_IRQ_OFFLOAD_VECTOR
DECL|TEST_SOFT_INT|macro|TEST_SOFT_INT
DECL|THREAD_STACK_SIZE|macro|THREAD_STACK_SIZE
DECL|dummy_isr|function|void dummy_isr(void *unused)
DECL|func_array|variable|func_array
DECL|i|variable|i
DECL|main|function|void main(void)
DECL|objects_thread|variable|objects_thread
DECL|pfunc|typedef|typedef void * (*pfunc) (void *);
DECL|thread_entry|function|static void thread_entry(void *arg1, void *arg2, void *arg3)
